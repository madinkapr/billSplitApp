...
export default function DesktopSummary({ bill, onReset, onSettleUp, onNewWithSameCrew }) {
  ...
  const results = calculateSplits({ items: bill.items || [], members: activeMembers, grandTotal: bill.grandTotal || 0 })
  ...
  function buildTextSummary() {
    ...
    lines.push('─'.repeat(32))
    results.forEach((r) => lines.push(`${r.name}${r.isMe ? t('common.you') : ''}: ${fmt(r.finalTotal)}`))
    lines.push('─'.repeat(32))
